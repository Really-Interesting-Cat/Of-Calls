...
void decode_gps_posllh_packet()
{
	...
	datag.iTow = gps_union.datag_Ulong;

	for (j = 0; j < 4; j++) // Longitude
		gps_union.buff[j] = gps_packet_buff[buff_idx++];
	datag.longitude = gps_union.datag_long;

	for (j = 0; j < 4; j++) // Latitude
		gps_union.buff[j] = gps_packet_buff[buff_idx++];
	datag.latitude = gps_union.datag_long;

	for (j = 0; j < 4; j++) // height
		gps_union.buff[j] = gps_packet_buff[buff_idx++];
	datag.height = gps_union.datag_long;

	for (j = 0; j < 4; j++) // hMSL
		gps_union.buff[j] = gps_packet_buff[buff_idx++];
	datag.hMSL = gps_union.datag_long;

	for (j = 0; j < 4; j++) // hAcc
		gps_union.buff[j] = gps_packet_buff[buff_idx++];
	datag.hAcc = gps_union.datag_Ulong;

	for (j = 0; j < 4; j++) // vAcc
		gps_union.buff[j] = gps_packet_buff[buff_idx++];
	datag.vAcc = gps_union.datag_Ulong;
}
